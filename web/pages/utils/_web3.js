...
const contractABI = require("/data/Arnhemia.json");

const acceptedChains = ENVIRONMENT === 'development' ? [1, 3, 4, 5, 42] : [1, 2];

export const arnhemia = new web3.eth.Contract(contractABI.abi, NFT_ADDRESS);
...
export const mintGift = async (account, proof) => {
  ...
  const result = arnhemia.methods.mintGift(proof).send({ from: account }).then((result) => {
      return {
        success: true,
...
export const mintWhitelist = async (account, proof) => {
  ...
  const result = arnhemia.methods.mintWhitelist(proof).send({ from: account, value: amountToWei }).then((result) => {
  ...
  export const mintPublic = async (account, numberOfTokens) => {
    ...
    const amountToWei = web3.utils.toWei(amount, 'ether');
    const result = arnhemia.methods.publicMint(numberOfTokens).send({ from: account, value: amountToWei }).then((result) => {
      console.log(`✅ Check out your transaction on Etherscan: https://mumbai.polygonscan.com/tx/` + result);
        return {
          success: true,
          status: `✅ Check out your transaction on Etherscan: https://mumbai.polygonscan.com/tx/` + result
          };
    }).catch((err) => {
      console.log("Mint transaction failed!");
      return {
