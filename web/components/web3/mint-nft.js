...
import { mintGift, mintPublic, mintWhitelist, arnhemia } from '@pages/utils/_web3';
...
const MintNFT = () => {
  ...
  useEffect(() => {
    ...
    async function checkIfClaimed() {
      arnhemia.methods.claimed(account).call({ from: account }).then((result) => {
  ...
  useEffect(() => {
    ...
    async function validateClaim() {
      arnhemia.methods.mintGift(giftProof).call({ from: account }).then(() => {
  ...
  useEffect(() => {
    ...
    async function validateClaim() {
      ...
      arnhemia.methods.mintWhitelist(whitelistProof).call({ from: account, value: amountToWei }).then(() => {
